...
## Requirements
### Requirement: 多奇標籤
系統 SHALL 提供「多奇」標籤，用於篩選多奇教育訓練會員專屬範本。

#### Scenario: 多奇標籤位置
- **WHEN** 使用者查看「我的範本」側邊欄的標籤列
- **THEN** 「多奇」標籤位於「全部」之後
- **AND** 「多奇」標籤位於「社群」之前
...
從 GitHub Issue 同步的範本 SHALL 自動包含「社群」標籤。

#### Scenario: 同步範本標籤
- **WHEN** 系統從 GitHub Issue 同步範本
- **THEN** 同步的範本自動包含「社群」標籤
- **AND** 保留 Issue 中原有指定的其他標籤

---

### Requirement: 提交範本自動標籤
使用者提交範本給作者時，系統 SHALL 確保範本包含「社群」標籤。

#### Scenario: 提交時自動加入社群標籤
- **WHEN** 使用者透過「提交給作者」功能送出範本
- **AND** 範本未包含「社群」標籤
