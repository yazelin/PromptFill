...
分享 URL SHALL 使用 hash fragment 編碼模板資料，格式為 `{baseURL}#template={base64編碼}&{變數名}={值}&...`。

#### Scenario: URL 產生格式
- **WHEN** 系統產生分享 URL
- **THEN** URL 包含 `#template=` 參數，值為 Base64 編碼的模板 JSON
- **AND** URL 包含所有變數選擇，格式為 `&變數名=值`

#### Scenario: 模板 JSON 內容
- **WHEN** 編碼模板 JSON
- **THEN** JSON 包含 `name`（模板名稱）
- **AND** JSON 包含 `content`（模板內容）
- **AND** JSON 包含 `selections`（變數選擇）

---

### Requirement: 分享 URL 解析
系統 SHALL 在頁面載入時解析 URL hash fragment，識別分享的模板資料。

#### Scenario: 載入分享 URL
- **WHEN** 使用者開啟包含分享參數的 URL
- **THEN** 系統解析 `#template=` 參數
- **AND** 解碼 Base64 取得模板資料
- **AND** 解析其他參數取得變數選擇
...
### Requirement: 分享預覽模式
系統 SHALL 在偵測到分享 URL 時進入預覽模式，顯示分享的模板但不自動匯入。
...
### Requirement: 匯入分享模板
使用者 SHALL 能透過匯入按鈕將分享的模板儲存到本地。

#### Scenario: 點擊匯入按鈕
- **WHEN** 使用者在預覽模式點擊匯入按鈕
- **THEN** 系統將分享的模板加入本地模板列表
- **AND** 切換到新匯入的模板
...
#### Scenario: 匯入模板儲存
- **WHEN** 模板匯入成功
- **THEN** 模板儲存到 localStorage
- **AND** 模板 ID 為新產生的唯一 ID
