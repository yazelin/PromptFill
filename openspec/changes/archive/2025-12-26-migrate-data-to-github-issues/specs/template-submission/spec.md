...
系統 SHALL 允許用戶將自訂範本提交給維護者審核，使用標準化的 Issue Template 格式。

#### Scenario: 用戶提交範本
- **WHEN** 用戶點擊「提交範本」按鈕
- **THEN** 系統複製範本資料到剪貼板（JSON 格式）
- **AND** 開啟 GitHub Issue 頁面，預填標準化格式

#### Scenario: Issue Template 格式
- **WHEN** GitHub Issue 頁面開啟
- **THEN** Issue 使用 `template-submission` 範本
- **AND** 包含以下欄位：
  - 範本名稱 (中文)
...
系統 SHALL 提供 GitHub Issue Template 用於範本提交。
